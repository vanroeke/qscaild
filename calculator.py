...
#    subprocess.run([mpirun]+["-f" ,os.environ.get("PBS_NODEFILE"), "-np", os.environ.get("PBS_NP"), "-bootstrap", "ssh", python_exe, os.path.join(qscaild_path,"submit_qscaild.py")],stdout=sys.stdout, stderr=sys.stderr,check=True)
#    return
...
#def qscaild():
    print('Running QSCAILD iteration')
    subprocess.run([mpirun]+[python_exe], os.path.join(qscaild_path,"submit_qscaild.py")],stdout=sys.stdout, stderr=sys.stderr,check=True)
#    return

#def vasprun(directory):
#    subprocess.run([mpirun]+["-f" ,os.environ.get("PBS_NODEFILE"), "-np", os.environ.get("PBS_NP"), "-bootstrap", "ssh", vasp_exe], cwd=directory ,shell=False, env=vasp_env, stdout=sys.stdout, stderr=sys.stderr)
#    return
...
#    subprocess.run([mpirun, "-f" ,os.environ.get("PBS_NODEFILE"), "-np", os.environ.get("PBS_NP"), "-bootstrap", "ssh", mlip_exe, "train", MLIP_potential, MLIP_train_set, "--force-weight=1.0", "--energy-weight=0.1", "--stress-weight=1.0", "--update-mindist", "--max-iter=2000"], env=mlip_env,stdout=sys.stdout, stderr=sys.stderr)
