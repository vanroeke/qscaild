...
def write_ifcs(phifull, poscar, sposcar, dmin, nequi, shifts, frange,
    ...
    for ii, jj in itertools.product(range(natoms), range(ntot)):
        ...
        for kk in range(ntot):
            ...
            nblocks += 1
            Rj = np.dot(
                sposcar["lattvec"], best2 + sposcar["positions"][:, jj] -
                sposcar["positions"][:, jatom])
            Rk = np.dot(
                sposcar["lattvec"], best3 + sposcar["positions"][:, kk] -
